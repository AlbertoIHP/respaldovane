...
export class AppComponent
{
  ...
  getNotifications()
  {
       this.notificationService.index().subscribe( data => {
        data = data._embedded.notifications
        data = data.filter( noti => noti.user_id === this.currentUser.id.toString() )
        ...
        if( data.length >= 1 )
        {
          ...
          {
            this.notificationTypeService.show( noti.notificationtype_id ).subscribe( info => {

              noti.link = info.redirect
            })
          }
